...
- 2026-05-12 — **Phase 4.6 composition + grid shipped** on `phase-4.6-composition` branch (9 commits). Full type scale tokenized (`--text-body-{sm,base,lg}` + `--text-display-{sm,md,lg,xl}` as fluid clamp values per §5.2). Five composition primitives in `src/components/composition/`: VerticalLabel, SectionNumber, PullQuote, Marginalia, SectionRule. Asymmetric grid utilities in `src/styles/grid.css`: `.grid-cartesian` (12-col, 1280 px max) + `.grid-tropical` (8-col, 960 px max) with span helpers. Every page recomposed onto the grid. Homepage hero stacks **Théo / Alessandro / Hermann** on three lines, Fraunces display-xl, italic on the third — wordmark stays in Fraunces (§ 2 intact). `/about` flipped Cartesian → Tropical (resolves the long-standing § 5.5 inconsistency). Project pages now render as four numbered sections (01 Context · 02 Method · 03 Findings & links · 04 Credit) inside a VerticalLabel-flanked spread. Lighthouse 100/100/100/100 on /, /work, /work/amsterdam-tree-shade, /about. A11y fixes during the audit: `--ink-muted` darkened from `#7a7570` → `#706b64` (4.7 : 1); terra demoted to non-text-only (PullQuote frame, gradient); oxblood promoted to Tropical text accent; cool-grey assigned to Cartesian project-card hover border (too light for text on Paper); FeaturedStrip section heading promoted to `<h2>`; `/work` project rows promoted to `<h2>`.
- 2026-05-14 — **Phase 4.7 Strip / Amplify / Add shipped** on `phase-4.7-strip-amplify-add` branch (22 commits). Design-only pass; content still deferred. **Strip** (S0–S5): `astro.config.mjs` site set to `https://theoalessandro.com` (also synced `siteConfig.url` to match — was poisoning canonical/OG/JSON-LD URLs); removed redundant per-page MonoRow eyebrows from every subpage; fixed title duplication on project pages (was rendered in both `<h1>` and `.hero-placeholder`); consolidated project metadata to exactly two surfaces (Breadcrumb + title-block sub-grid with StatusPill at cols 10–12 + Updated line); deleted About's H2 "Credit" section; footer dedupe audit passed clean. **Amplify** (A1–A4): aggressive multi-line H1s site-wide via new `--text-hero` (clamp 64–168 px), extended `--text-display-xl` (40–72 px) and `--text-display-lg` (36–64 px) tokens. Homepage wordmark moved from Fraunces to **Space Grotesk** with italic `<em>` on the middle line "Alessandro" (Phase 4.6 option (b) superseded); About wordmark stays Fraunces with `opsz 36 / SOFT 60 (100, WONK 1 italic)` — register divergence reinforced. Pull quote primitive promoted to a typographic event (Fraunces italic, `opsz 48 / SOFT 100 / WONK 1`, terra border, half-viewport breathing room); two pull quotes on About now. Homepage hero is **90vh asymmetric**: wordmark at cols 1–8, meta column at cols 10–12 offset down by `var(--s-20)`, scroll signal at viewport bottom — featured strip moved to `.below-hero`. `/work` index gets a 60vh composed featured block + numbered editorial list (`work-row-num` 02, 03…). **Add** (D1–D10): new `<Figure>` primitive with designed `pending` empty state — wired to every project hero, project card thumbnail, and featured tile (no more gray rectangles); `StatusPill` extended with `size="sm"` + `tone="cool-grey"` border accent (cool-grey border + ink-muted text for AA); new `<TagPill>` and `.tag-row` (replaces flat slashed tag strings); new `<DataRow>` primitive for label/value pairs; About has sticky **Marginalia** at cols 7–8 (Languages / Trajectory / CV / Contact via DataRow). Project page sections are now per-section `.grid-cartesian` sub-grids: VerticalLabel at col 1, big terra SectionNumber (`--text-display-md` weight 300) at col 2, body at cols 3–9 — both sticky. Credit section redesigned as designed-empty-state blocks (`⧉  Collaborators surfaced on publication` etc. — empty states are a design surface, per Anna). Header **compacts on scroll** via IntersectionObserver hooked into BaseLayout (re-runs on `astro:page-load` for view transitions). End-of-document signal `END · 04 SECTIONS · DATE` between Findings and Credit. Tropical register + Fraunces axis audits ran clean except: Tropical `.prose p` paragraph spacing bumped to `var(--s-6)` and `.card-title` (Tropical) added the missing `SOFT` axis. **Grid namespace refactor**: classes renamed from half-open `col-1-9` (cols 1–8) to inclusive `col-1-8` (cols 1–8); `.col-t-*` Tropical-prefix dropped — a single `col-X-Y` namespace works in either grid. **Phase 4 self-audit passed all six categories.**
