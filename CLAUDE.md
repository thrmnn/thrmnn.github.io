...
- 2026-09-03 — Domain cutover: `theoalessandro.com` is the site (GitHub Pages custom domain, cert for apex + www, HTTPS enforced; DNS-only A/AAAA at Cloudflare; `thrmnn.github.io` redirects). Plausible `data-domain` stays `thrmnn.github.io` until the new site exists in Plausible. Review surface `preview.theoalessandro.com` = the vitrine through the hetzner Cloudflare tunnel behind an Access app.
